...
class Training:
    ...
    def train_evaluate(self, model_name): # read config
        # check GPU
        ...
        if model_name == 'en_trf_bertbaseuncased_lg' or 'en_trf_xlnetbasecased_lg':
        ...
        if self.train_path and self.dev_path:
            try:
                train_texts, train_cats = ClassificationDataReader.read_inputs(self.train_path)
                dev_texts, dev_cats = ClassificationDataReader.read_inputs(self.dev_path)
            except ValueError:
